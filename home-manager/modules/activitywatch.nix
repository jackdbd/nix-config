...
with lib; let
  ...
  cleanup-activitywatch-logs = pkgs.writeShellScriptBin "cleanup-activitywatch-logs" ''
    ...
    cleanup() {
      local days=3
      local subdir=$1
      local logfiles=$(${find} ${logBase}/$subdir/ -mtime +$days)

      if [[ -n "$logfiles" ]]; then
        ${printf} "cleanup log files older than $days days in ${logBase}/$subdir/\n"
        ${rm} $logfiles
      else
        ${printf} "no log files older than $days days in ${logBase}/$subdir/\n"
      fi
    }

    # Example:
    # find ~/.cache/activitywatch/log/aw-watcher-afk/ -mtime +5 | xargs rm

    cleanup aw-qt
    cleanup aw-server-rust
    cleanup aw-watcher-afk
    cleanup aw-watcher-window
  '';
in {
  meta = {};
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services.activitywatch-cleanup-logs = {
      Unit = {
        Description = "Remove old log files generated by ActivityWatch modules";
      };
      Service = {
        ExecStart = "${cleanup-activitywatch-logs}/bin/cleanup-activitywatch-logs";
        Restart = "no";
        Type = "oneshot";
      };
    };

    systemd.user.timers.activitywatch-cleanup-logs = {
      Unit = {
        Description = "Periodically triggers activitywatch-cleanup-logs.service";
      };
      Timer = {
        OnCalendar = "daily";
      };
